...
const AcademyBookScreen = ({navigation}) => {
  ...
  const getAcabookdata = async (userId) => {
    ...
    if (response.status === 200) {
      ...
      if (responseJson.status === 'success') {
        // console.log(responseJson.data.bookInfo);
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      ...
      headerTitle: () => <Text style={styles.title}>학원 문제집</Text>,
      headerLeft: () => (
        <TouchableOpacity
          onPress={() => {
            {
...
export default AcademyBookScreen;
